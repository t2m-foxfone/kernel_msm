...
static ssize_t stk_ps_prx_raw_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct stk3x1x_data *ps_data =  dev_get_drvdata(dev);	
    uint32_t reading;
    reading = stk3x1x_get_ps_reading(ps_data);
...
static struct device_attribute ps_prx_raw_attribute = __ATTR(prx_raw, 0444, stk_ps_prx_raw_show, NULL);
                        ...
                        const struct i2c_device_id *id)
{
	...
	device_create_file(&(client->dev),&ps_prx_raw_attribute);
